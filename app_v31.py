...
def _build_resumo_metricas(
    ...
    valor_so_b1 = soma_comp(f"Chave só na {base1_name}")
    valor_so_b2 = soma_comp(f"Chave só na {base2_name}")

    valor_divergente = 0.0
    if not ponte.empty:
        mask = ponte["Agrupador"].eq("TOTAL GERAL") & ponte["Componente"].eq(f"Valor divergente entre {base1_name} e {base2_name}")
        valor_divergente = float(ponte.loc[mask, "Valor"].sum())

    impacto_abs = 0.0
    if not ponte.empty:
        mask = ponte["Agrupador"].eq("TOTAL GERAL")
        impacto_abs = float(ponte.loc[mask, "Valor"].abs().sum())

    diferenca_liquida = valor_so_b1 + valor_so_b2 + valor_divergente
    ...
    metricas = pd.DataFrame(
        [
            ...
            {"Indicador": f"Valor só na {base1_name}", "Valor": valor_so_b1},
            {"Indicador": f"Valor só na {base2_name}", "Valor": valor_so_b2},
            {"Indicador": "Diferença líquida", "Valor": diferenca_liquida},
...
def _build_metricas_semantic_map(metricas: pd.DataFrame) -> Dict[str, dict]:
    ...
    for _, row in metricas.iterrows():
        ...
        if ind in [
            ...
            out[ind] = {"tipo_logico": "moeda", "casas_decimais": 2, "formato_excel": "R$ #,##0.00"}

    return {
        "Indicador": {"tipo_logico": "texto", "casas_decimais": 0, "formato_excel": ""},
        "Valor": {"tipo_logico": "texto", "casas_decimais": 0, "formato_excel": ""},
        "__linha__": out,
    }
...
def _write_metricas_block(ws, wb, start_row: int, start_col: int, title: str, df: pd.DataFrame, semantic_map: Dict[str, dict]):
    ...
    for i, row in df.reset_index(drop=True).iterrows():
        ...
        ws.write(cell_row, start_col, indicador)
