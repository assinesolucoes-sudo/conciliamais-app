...
def render_cruzamento_inteligente_v2():
    ...
    def _apply_transform_value(x, mode, chars_n=None, prefix="", zfill_size=None):
        s = "" if pd.isna(x) else str(x).strip()
        if mode == "Sem tratamento":
            return s
        if mode == "Considerar apenas números":
            return re.sub(r"\D", "", s)
        if mode == "Ignorar zeros à esquerda":
            s2 = re.sub(r"\D", "", s)
            s2 = re.sub(r"^0+", "", s2)
            return s2 if s2 != "" else "0"
        if mode == "Considerar os primeiros caracteres":
            n = int(chars_n or 0)
            return s[:n] if n > 0 else s
        if mode == "Considerar os últimos caracteres":
            n = int(chars_n or 0)
            return s[-n:] if n > 0 else s
    ...
    def _apply_transform_series(sr, mode, chars_n=None, prefix="", zfill_size=None):
        return _force_text_series(sr).map(lambda x: _apply_transform_value(x, mode, chars_n, prefix, zfill_size))
    ...
    def _compare_series(sa_raw, sb_raw, mode="Numérico", tol=0.01):
        ...
        if mode == "Numérico":
            n1 = sa_raw.map(normalize_money)
            n2 = sb_raw.map(normalize_money)
            both = n1.notna() & n2.notna()
            diff = pd.Series(np.nan, index=sa_raw.index, dtype="float64")
            diff.loc[both] = (n1.loc[both] - n2.loc[both]).round(2)
            ...
            return sa_raw, sb_raw, diff, status
        if mode == "Texto exato":
        ...
        for r in range(len(df)):
            for c, col in enumerate(df.columns):
                ...
                if (
                    ...
                    or "STATUS" in col_up
                    or "BASE A_" in col_up
                    or "BASE B_" in col_up
                ):
                    ws.write_string(r + 1, c, str(val), fmt_text)
                elif isinstance(val, (int, float, np.integer, np.floating)) and ("DIF_" in col_up or "SOMA_" in col_up or "ADERENCIA" in col_up):
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_col):
        if df_result is None or df_result.empty or not totalizador_col or totalizador_col not in df_result.columns:
            return pd.DataFrame()
        work = df_result.copy()
        work["TOTALIZADOR"] = work[totalizador_col].fillna("").astype(str)
        agg = (
            work.groupby("TOTALIZADOR", dropna=False)
            .agg(
                QTDE_REGISTROS=("TOTALIZADOR", "size"),
                QTDE_MATCH_EXATO=("RESULTADO_FINAL", lambda s: int((s.astype(str) == "Match exato").sum())),
                ...
                QTDE_DUPLICIDADES=("RESULTADO_FINAL", lambda s: int((s.astype(str) == "Duplicidade").sum())),
            )
            .reset_index()
        )
        for meta in compare_meta:
            diff_col = meta["diff_col"]
            if diff_col in work.columns:
                tmp = pd.to_numeric(work[diff_col], errors="coerce").fillna(0.0)
                grp = work.assign(__TMP=tmp).groupby("TOTALIZADOR", dropna=False)["__TMP"].sum().reset_index()
                grp.columns = ["TOTALIZADOR", f"SOMA_{diff_col}"]
                agg = agg.merge(grp, on="TOTALIZADOR", how="left")
        agg = agg.rename(columns={"TOTALIZADOR": totalizador_col})
        return agg.sort_values(totalizador_col).reset_index(drop=True)

    def _to_excel_package(df_result, resumo_dict, text_priority_cols=None, totalizador_df=None):
        output = BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            wsr.write("D3", "Total", fmt_label); wsr.write("E3", resumo_dict.get("total", 0), fmt_kpi)
            wsr.write("D4", "Matches", fmt_label); wsr.write("E4", resumo_dict.get("encontrados", 0), fmt_kpi)
            wsr.write("D5", "Ausentes", fmt_label); wsr.write("E5", resumo_dict.get("nao_encontrados", 0), fmt_kpi)
            wsr.write("D6", "Divergências", fmt_label); wsr.write("E6", resumo_dict.get("divergentes", 0), fmt_kpi)
            wsr.write("D7", "Duplicidades", fmt_label); wsr.write("E7", resumo_dict.get("duplicados", 0), fmt_kpi)
            if totalizador_df is not None and not totalizador_df.empty:
                start_col = 6
                wsr.write(0, start_col, "Resumo por totalizador", fmt_hdr)
                for c, col in enumerate(totalizador_df.columns):
                    wsr.write(1, start_col + c, col, fmt_hdr)
                for r in range(len(totalizador_df)):
                    for c, col in enumerate(totalizador_df.columns):
                        val = totalizador_df.iloc[r, c]
                        if isinstance(val, (int, float, np.integer, np.floating)) and ("SOMA_" in str(col).upper()):
                            wsr.write_number(r + 2, start_col + c, float(val), fmt_num)
                        else:
                            wsr.write(r + 2, start_col + c, val, fmt_value)
            ...
            if not df_result.empty and "RESULTADO_FINAL" in df_result.columns:
                for nome, filtro in [
                    ...
                    ("NAO_ENCONTRADOS", df_result["RESULTADO_FINAL"] == "Sem correspondência"),
                    ("DUPLICIDADES", df_result["RESULTADO_FINAL"] == "Duplicidade"),
                ]:
                    df_tmp = df_result[filtro].copy()
                    if not df_tmp.empty:
                        ws = wb.add_worksheet(nome)
                        writer.sheets[nome] = ws
                        _write_df_excel(ws, df_tmp, wb, text_priority_cols=text_priority_cols)
        output.seek(0)
        return output

    def _prepare_prefixed(df, prefix):
        out = df.copy()
        ren = {c: f"{prefix}{c}" for c in out.columns if c != "__KEY__"}
        return out.rename(columns=ren)

    def _process_direction(base_a, base_b, selected_keys, selected_compares, origin_label, totalizador_choice):
        a_dup_keys = set(base_a["__KEY__"].value_counts()[lambda s: s > 1].index.tolist())
        b_dup_keys = set(base_b["__KEY__"].value_counts()[lambda s: s > 1].index.tolist())
        a_unique = base_a[~base_a["__KEY__"].isin(a_dup_keys)].copy()
        b_unique = base_b[~base_b["__KEY__"].isin(b_dup_keys)].copy()
        if origin_label == "A":
            merged = _prepare_prefixed(base_a, "A__").merge(_prepare_prefixed(b_unique, "B__"), on="__KEY__", how="left", indicator=True)
            dup_dest = b_dup_keys; base_validada = "Base A"; base_ref = "Base B"
        else:
            merged = _prepare_prefixed(base_b, "B__").merge(_prepare_prefixed(a_unique, "A__"), on="__KEY__", how="left", indicator=True)
            dup_dest = a_dup_keys; base_validada = "Base B"; base_ref = "Base A"
        result = pd.DataFrame(index=merged.index)
        result["BASE_VALIDADA"] = base_validada
        result["BASE_REFERENCIA"] = base_ref
        result["CHAVE_PROCESSADA"] = merged["__KEY__"].fillna("")
        in_dup = merged["__KEY__"].isin(dup_dest)
        found = merged["_merge"].eq("both")
        not_found = (~found) & (~in_dup)
        result["STATUS_MATCH"] = np.select([in_dup, found, not_found],[f"Duplicidade na {base_ref}", "Encontrado", "Não encontrado"], default="Não encontrado")
        result["RESULTADO_FINAL"] = np.select([in_dup, found, not_found],["Duplicidade", "Match exato", "Sem correspondência"], default="Sem correspondência")
        for _, key_row in selected_keys.iterrows():
            ca = f"CHAVE_BASE A_{key_row['CAMPO_BASE_A']}"
            cb = f"CHAVE_BASE B_{key_row['CAMPO_BASE_B']}"
            result[ca] = merged.get(f"A__{key_row['CAMPO_BASE_A']}", "").fillna("")
            result[cb] = merged.get(f"B__{key_row['CAMPO_BASE_B']}", "").fillna("")
        compare_meta = []
        has_div = pd.Series(False, index=result.index)
        for _, cp in selected_compares.iterrows():
            raw_a, raw_b, diff, status = _compare_series(
                merged.get(f"A__{cp['CAMPO_BASE_A']}", pd.Series("", index=merged.index)),
                merged.get(f"B__{cp['CAMPO_BASE_B']}", pd.Series("", index=merged.index)),
                mode=cp["TIPO"], tol=float(cp["TOLERANCIA"] or 0.0)
            )
            ...
            result[c_df] = diff
            result[c_st] = status
            has_div = has_div | (found & status.eq("Divergente"))
            compare_meta.append({"diff_col": c_df})
        result.loc[found & has_div, "RESULTADO_FINAL"] = "Match com divergência"
        totalizador_col_result = None
        if totalizador_choice and totalizador_choice != "(não gerar resumo por totalizador)":
            lado, campo = totalizador_choice.split(" :: ", 1)
            pref = "A__" if lado == "Base A" else "B__"
            totalizador_col_result = f"TOTALIZADOR_{lado}_{campo}"
            result[totalizador_col_result] = merged.get(f"{pref}{campo}", "").fillna("").astype(str)
        return result, compare_meta, totalizador_col_result
    ...
    direcao = st.radio("Como deseja comparar as bases?", [
    ...
    if sug_df.empty:
        key_rows = [{"USAR": True, "ORDEM": 1, "CAMPO_BASE_A": list(df_a.columns)[0], "CAMPO_BASE_B": list(df_b.columns)[0], "CONFIANCA": "Manual", "SCORE": 0.0}]
    else:
    ...
    selected_keys = selected_keys[selected_keys["CAMPO_BASE_A"].isin(df_a.columns) & selected_keys["CAMPO_BASE_B"].isin(df_b.columns)].copy()
    ...
    st.caption("Use esta etapa para padronizar campos como filial, código, plaqueta e patrimônio antes do match.")
    treatment_rows = []
    for _, r in selected_keys.iterrows():
        treatment_rows.append({
            "CAMPO_BASE_A": r["CAMPO_BASE_A"], "CAMPO_BASE_B": r["CAMPO_BASE_B"], "TRATAR": False,
            "TRATAMENTO_BASE_A": "Sem tratamento", "CARACTERES_A": 0, "PREFIXO_A": "", "ZEROS_A": 0,
            "TRATAMENTO_BASE_B": "Sem tratamento", "CARACTERES_B": 0, "PREFIXO_B": "", "ZEROS_B": 0,
        })
    treatment_df = st.data_editor(pd.DataFrame(treatment_rows), use_container_width=True, height=220, hide_index=True,
        column_config={
            "CAMPO_BASE_A": st.column_config.TextColumn("Campo Base A", disabled=True),
            "CAMPO_BASE_B": st.column_config.TextColumn("Campo Base B", disabled=True),
            "TRATAR": st.column_config.CheckboxColumn("Tratar"),
            "TRATAMENTO_BASE_A": st.column_config.SelectboxColumn("Tratamento Base A", options=["Sem tratamento","Considerar apenas números","Ignorar zeros à esquerda","Considerar os primeiros caracteres","Considerar os últimos caracteres","Prefixo + zeros à esquerda"]),
            "CARACTERES_A": st.column_config.NumberColumn("Caracteres A", min_value=0, step=1),
            "PREFIXO_A": st.column_config.TextColumn("Prefixo A"),
            "ZEROS_A": st.column_config.NumberColumn("Zeros A", min_value=0, step=1),
            "TRATAMENTO_BASE_B": st.column_config.SelectboxColumn("Tratamento Base B", options=["Sem tratamento","Considerar apenas números","Ignorar zeros à esquerda","Considerar os primeiros caracteres","Considerar os últimos caracteres","Prefixo + zeros à esquerda"]),
            "CARACTERES_B": st.column_config.NumberColumn("Caracteres B", min_value=0, step=1),
            "PREFIXO_B": st.column_config.TextColumn("Prefixo B"),
            "ZEROS_B": st.column_config.NumberColumn("Zeros B", min_value=0, step=1),
        }, key="treatment_grid_v13")

    st.markdown("### 6) Quais campos deseja validar para saber se os valores são coerentes?")
    st.caption("O resultado sairá em uma única tabela, sempre na ordem Base A, Base B e Diferença.")
    ...
    if not compare_rows:
        compare_rows = [{"COMPARAR": True, "ORDEM": 1, "CAMPO_BASE_A": list(df_a.columns)[0], "CAMPO_BASE_B": list(df_b.columns)[0], "TIPO": "Numérico", "TOLERANCIA": 0.00}]
    ...
    selected_compares = selected_compares[selected_compares["CAMPO_BASE_A"].isin(df_a.columns) & selected_compares["CAMPO_BASE_B"].isin(df_b.columns)].copy()
    ...
    totalizador_options = ["(não gerar resumo por totalizador)"] + [f"Base A :: {c}" for c in df_a.columns] + [f"Base B :: {c}" for c in df_b.columns]
    totalizador_choice = st.selectbox("Campo para totalização no resumo executivo", totalizador_options, index=0, help="Exemplo: filial. Esse campo não fica fixo no fonte; o usuário escolhe a cada processamento.")

    st.markdown("### 8) Processar análise")
    processar = st.button("Executar análise", type="primary", use_container_width=True)
    if not processar:
        return

    with st.spinner("Processando análise..."):
        base_a = df_a.copy(); base_b = df_b.copy()
        for c in base_a.columns: base_a[c] = _force_text_series(base_a[c])
        for c in base_b.columns: base_b[c] = _force_text_series(base_b[c])
        key_cols_a, key_cols_b = [], []
        for i, (_, key_row) in enumerate(selected_keys.iterrows(), start=1):
            tr = treatment_df[(treatment_df["CAMPO_BASE_A"] == key_row["CAMPO_BASE_A"]) & (treatment_df["CAMPO_BASE_B"] == key_row["CAMPO_BASE_B"])]
            tr = tr.iloc[0] if len(tr) else None
            mode_a = tr["TRATAMENTO_BASE_A"] if tr is not None and bool(tr["TRATAR"]) else "Sem tratamento"
            mode_b = tr["TRATAMENTO_BASE_B"] if tr is not None and bool(tr["TRATAR"]) else "Sem tratamento"
            col_key_a = f"__KEY_A_{i}"; col_key_b = f"__KEY_B_{i}"
            base_a[col_key_a] = _apply_transform_series(base_a[key_row["CAMPO_BASE_A"]], mode_a, chars_n=0 if tr is None else tr["CARACTERES_A"], prefix="" if tr is None else tr["PREFIXO_A"], zfill_size=0 if tr is None else tr["ZEROS_A"])
            base_b[col_key_b] = _apply_transform_series(base_b[key_row["CAMPO_BASE_B"]], mode_b, chars_n=0 if tr is None else tr["CARACTERES_B"], prefix="" if tr is None else tr["PREFIXO_B"], zfill_size=0 if tr is None else tr["ZEROS_B"])
            key_cols_a.append(col_key_a); key_cols_b.append(col_key_b)
        ...
        if direcao == "Validar Base A contra Base B":
            df_result, compare_meta, totalizador_col_result = _process_direction(base_a, base_b, selected_keys, selected_compares, "A", totalizador_choice)
        elif direcao == "Validar Base B contra Base A":
            df_result, compare_meta, totalizador_col_result = _process_direction(base_a, base_b, selected_keys, selected_compares, "B", totalizador_choice)
        else:
            df_r1, compare_meta, totalizador_col_result = _process_direction(base_a, base_b, selected_keys, selected_compares, "A", totalizador_choice)
            df_r2, _, _ = _process_direction(base_a, base_b, selected_keys, selected_compares, "B", totalizador_choice)
            df_result = pd.concat([df_r1, df_r2], ignore_index=True)
        ...
        key_order_cols = []
        for _, key_row in selected_keys.iterrows():
            ca = f"CHAVE_BASE A_{key_row['CAMPO_BASE_A']}"; cb = f"CHAVE_BASE B_{key_row['CAMPO_BASE_B']}"
            if ca in df_result.columns: key_order_cols.append(ca)
            if cb in df_result.columns: key_order_cols.append(cb)

        compare_order_cols = []
        for _, cp in selected_compares.iterrows():
            cols_cmp = [f"VALOR_BASE A_{cp['CAMPO_BASE_A']}", f"VALOR_BASE B_{cp['CAMPO_BASE_B']}", f"DIF_{cp['CAMPO_BASE_A']}__{cp['CAMPO_BASE_B']}", f"STATUS_{cp['CAMPO_BASE_A']}__{cp['CAMPO_BASE_B']}"]
            for c in cols_cmp:
                if c in df_result.columns: compare_order_cols.append(c)

        final_cols = [c for c in ["BASE_VALIDADA", "BASE_REFERENCIA", "CHAVE_PROCESSADA"] if c in df_result.columns]
        final_cols.extend(key_order_cols)
        if totalizador_col_result and totalizador_col_result in df_result.columns: final_cols.append(totalizador_col_result)
        final_cols.extend(compare_order_cols)
        final_cols.extend([c for c in ["STATUS_MATCH", "RESULTADO_FINAL"] if c in df_result.columns])
        ...
        resumo_dict = {"objetivo": objetivo_label, "direcao": direcao, "total": total, "encontrados": encontrados, "nao_encontrados": nao_encontrados, "duplicados": duplicados_result, "divergentes": divergentes, "aderencia": round(aderencia, 2)}
        totalizador_df = _build_totalizador(df_result, compare_meta, totalizador_col_result) if gerar_resumo_exec else pd.DataFrame()
        text_priority_cols = [c for c in df_result.columns if any(x in c.upper() for x in ["CHAVE", "COD", "CÓD", "FILIAL", "LOJA", "PATRIM", "PLAQUETA"])]
        excel_bytes = _to_excel_package(df_result=df_result, resumo_dict=resumo_dict, text_priority_cols=text_priority_cols, totalizador_df=totalizador_df)

    st.markdown("### Resultado da análise")
    m1, m2, m3, m4, m5 = st.columns(5)
    with m1: st.metric("Registros analisados", total)
    with m2: st.metric("Correspondências", encontrados)
    with m3: st.metric("Ausentes", nao_encontrados)
    with m4: st.metric("Duplicidades", duplicados_result)
    with m5: st.metric("Divergências", divergentes)
    ...
    if gerar_resumo_exec and totalizador_df is not None and not totalizador_df.empty:
        st.markdown("#### Resumo executivo por totalizador")
        st.dataframe(totalizador_df, use_container_width=True, height=260)
    st.download_button("Baixar resultado em Excel", data=excel_bytes, file_name=f"Match_Inteligente_V4_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
