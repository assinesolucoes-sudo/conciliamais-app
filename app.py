...
def render_cruzamento_inteligente_v2():
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols):
        ...
        for meta in compare_meta:
            compare_related.update([meta.get("val_a_col"), meta.get("val_b_col"), meta.get("diff_col")])
        raw_group_cols = [c for c in totalizador_cols if c in df_result.columns and c not in compare_related]
        if not raw_group_cols:
            raw_group_cols = [c for c in df_result.columns if c.startswith("CHAVE_")][:3]
        if not raw_group_cols:
            return pd.DataFrame(), []

        work = df_result.copy()
        canonical_group_cols = []
        label_used = set()
        for i, c in enumerate(raw_group_cols, start=1):
            label = _label_exec(c)
            if label in label_used:
                continue
            dup_cols = [x for x in raw_group_cols if _label_exec(x) == label]
            new_col = f"__GRP__{i}"
            grp_df = work[dup_cols].replace("", np.nan)
            work[new_col] = grp_df.bfill(axis=1).iloc[:, 0].fillna("")
            canonical_group_cols.append((new_col, label))
            label_used.add(label)

        group_cols = [c for c, _ in canonical_group_cols]
        agg_rows = []
        for keys, grp in work.groupby(group_cols, dropna=False):
            if len(group_cols) == 1:
                keys = (keys,)
            row = {label: keys[i] for i, (_, label) in enumerate(canonical_group_cols)}
            ...
            for meta in compare_meta:
                ...
                vd = pd.to_numeric(grp.get(meta.get("diff_col")), errors="coerce").fillna(0).round(2).sum()
                row[f"{label} {nome_base_a if 'nome_base_a' in locals() else 'Base A'}"] = round(float(va), 2)
                row[f"{label} {nome_base_b if 'nome_base_b' in locals() else 'Base B'}"] = round(float(vb), 2)
                row[f"Diferença {label}"] = round(float(vd), 2)
            agg_rows.append(row)
        ...
        for c in diff_cols:
            total_diff = round(float(pd.to_numeric(agg[c], errors="coerce").fillna(0).sum()), 2)
            if total_diff != 0:
                financial_kpis.append((c, total_diff))
        if diff_cols:
            mask_non_zero = agg[diff_cols].apply(pd.to_numeric, errors="coerce").fillna(0).abs().sum(axis=1).round(2) != 0
        ...
        if support_cols:
            mask_support = agg[support_cols].apply(pd.to_numeric, errors="coerce").fillna(0).sum(axis=1) > 0
            if mask_support.any():
                agg = agg[mask_support].copy()
        if not agg.empty:
            sort_cols = [label for _, label in canonical_group_cols if label in agg.columns]
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V10_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
