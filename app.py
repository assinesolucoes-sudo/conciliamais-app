...
def render_cruzamento_inteligente_v2():
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            if totalizador_df is not None and not totalizador_df.empty:
                ...
                for c, col in enumerate(totalizador_df.columns):
                    ...
                    ws.set_column(c, c, width)
                money_cols = [c for c in totalizador_df.columns if str(c).startswith("Diferença ") or str(c).endswith(f" {nome_base_a}") or str(c).endswith(f" {nome_base_b}")]
    ...
    with gx:
        group_choices = st.multiselect(
            "Agrupar resumo por",
            group_options,
            default=default_group_choices,
            help="Use aqui os campos-chave que explicam onde está a diferença. Eles já vêm sugeridos a partir do match.",
            key="v16_group_choices",
        )
    with vx:
        value_choices = st.multiselect(
            "O que deseja totalizar/confrontar",
            compare_value_options,
            default=compare_value_options,
            help="Esses são os campos de valor cujo total e diferença serão exibidos no resumo executivo.",
            key="v16_value_choices",
        )
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V16_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
