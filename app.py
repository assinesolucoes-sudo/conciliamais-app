...
.cm-card .k { font-size: 12px; opacity: .75; margin-bottom: 8px; color:#334155; }
.cm-card .v { font-size: 22px; font-weight: 800; color:#0F172A; }
.cm-card .s { font-size: 12px; opacity: .75; margin-top: 6px; color:#334155; }
...
.cm-ok   { background:#ECFDF5; color:#047857; border-color:#A7F3D0; }
.cm-warn { background:#FFFBEB; color:#B45309; border-color:#FDE68A; }
.cm-bad  { background:#FEF2F2; color:#B91C1C; border-color:#FCA5A5; }
...
.cm-help { opacity: .75; font-size: 13px; margin-top: -6px; color:#334155; }
...
.cm-mini .k { font-size: 12px; opacity: .75; margin-bottom: 4px; color:#334155; }
.cm-mini .v { font-size: 20px; font-weight: 900; letter-spacing: -0.01em; color:#0F172A; }
...
.cm-detail .title { font-size: 14px; font-weight: 900; margin-bottom: 10px; color:#0F172A; }
.cm-detail .row { font-size: 13px; margin: 4px 0; color:#0F172A; }
.cm-detail .label { opacity: .70; color:#334155; }
.cm-detail .val { font-weight: 700; color:#0F172A; }
...
# Motivo fixo por núcleo (1 pra 1)
NUCLEO_MOTIVO = {
    "Processo interno": "Processo interno — revisar execução (baixa/estorno/cancelamento) e consistência do processo",
    "Cadastro": "Cadastro — financeiro sem contabilização / revisar natureza/parametrização contábil",
    "Configuração RP": "Configuração RP — possível falha/ausência de rotina (RP) / revisar parametrização e execução",
    "Não identificado": "Não identificado — revisar caso e ajustar classificação",
}

def severidade_por_valor(v):
    try:
        x = abs(float(v))
    except Exception:
        return "Normal"
...
# ============================
# Excel (corrigido: cabeçalho claro + fonte preta)
# ============================
...
def to_excel_divergencias_filtradas(df_filtrado, total_filtrado, total_aberto, filtros, stats, generated_at):
    ...
    with pd.ExcelWriter(out, engine="xlsxwriter") as w:
        ...
        fmt_txt = wb.add_format({"border": 1, "font_color": "#0F172A"})
        fmt_hdr = wb.add_format({
            "bold": True, "border": 1,
            "align": "center", "valign": "vcenter",
            "bg_color": "#EEF2FF", "font_color": "#0F172A"
        })
        fmt_date = wb.add_format({"num_format": "dd/mm/yyyy", "border": 1, "font_color": "#0F172A"})
        fmt_money = wb.add_format({"num_format": 'R$ #,##0.00;[Red]-R$ #,##0.00', "border": 1, "font_color": "#0F172A"})
        ...
        if nrows > 0 and ncols > 0:
            ...
            ws.add_table(
                ...
                {"style": "Table Style Light 9", "columns": columns, "autofilter": True}
            )
        ...
        ws3.set_column(1, 1, 26, wb.add_format({"num_format": 'R$ #,##0.00;[Red]-R$ #,##0.00', "font_color": "#0F172A"}))
...
if st.session_state.page == "upload":
    ...
    # Régua de etapas (simples e clara)
    st.progress(1/4)
    st.markdown('<div class="cm-help">Etapa 1 de 4 — Upload dos arquivos</div>', unsafe_allow_html=True)
    ...
    st.progress(2/4)
    st.markdown('<div class="cm-help">Etapa 2 de 4 — Mapeamento de colunas</div>', unsafe_allow_html=True)
    ...
    st.progress(3/4)
    st.markdown('<div class="cm-help">Etapa 3 de 4 — Validação (saldo anterior)</div>', unsafe_allow_html=True)
    ...
    st.progress(4/4)
    st.markdown('<div class="cm-help">Etapa 4 de 4 — Processamento</div>', unsafe_allow_html=True)
    ...
    if st.button("Processar e ir para Resultados", type="primary", disabled=not proceed_ok):
        ...
        # Financeiro: DOCUMENTO prioriza prefixo/título
        ...
        # Contábil: tenta extrair documento do histórico
        ...
        # Núcleo sugerido e motivo fixo
        div["NUCLEO_CONFIRMADO"] = div.apply(suggest_nucleo, axis=1)
        div["MOTIVO_CONFIRMADO"] = div["NUCLEO_CONFIRMADO"].map(lambda x: NUCLEO_MOTIVO.get(x, NUCLEO_MOTIVO["Não identificado"]))
        div["OBS_USUARIO"] = ""
        div["STATUS"] = "Pendente"
        div["RESOLVIDO"] = False

        # Severidade + Origem exibida com sinal embutido
        div["SEVERIDADE"] = div["VALOR"].map(severidade_por_valor)
        div["ORIGEM_EXIBICAO"] = div.apply(lambda r: f"{r['SEVERIDADE']} • {r['ORIGEM']}", axis=1)
...
# ============================
    ...
    # Filtros (limpos)
    # ========================
    f1, f2, f3, f4 = st.columns([1.15, 1.05, 2.3, 1.1])
    with f1:
    ...
    with f3:
        busca = st.text_input("Buscar (documento, histórico, chave, núcleo, obs)", value="")
    with f4:
        quick = st.selectbox("Atalho", ["(nenhum)", "Somente Crítica", "Atenção + Crítica"], index=0)
    ...
    resolved_mask = df["RESOLVIDO"] | (df["STATUS"].astype(str).str.lower().eq("resolvido"))
    if ver == "Somente em aberto":
        df = df[~resolved_mask].copy()
    elif ver == "Somente resolvidas":
        df = df[resolved_mask].copy()

    if quick == "Somente Crítica":
        df = df[df["SEVERIDADE"].eq("Crítica")].copy()
    elif quick == "Atenção + Crítica":
        df = df[df["SEVERIDADE"].isin(["Atenção", "Crítica"])].copy()

    if busca.strip():
        q = busca.strip().lower()
        cols_search = ["DOCUMENTO", "HISTORICO_OPERACAO", "CHAVE_DOC", "NUCLEO_CONFIRMADO", "MOTIVO_CONFIRMADO", "OBS_USUARIO", "ORIGEM_EXIBICAO"]
        mask = False
    ...
    # ========================
    # KPIs (sempre baseados no MASTER, não no filtro)
    # ========================
    master_resolved = div_master["RESOLVIDO"] | (div_master["STATUS"].str.lower().eq("resolvido"))
    total_itens = int(len(div_master))
    itens_res = int(master_resolved.sum())
    itens_ab = int(total_itens - itens_res)

    # VALOR EM ABERTO = soma apenas dos não resolvidos (isso reduz na hora)
    valor_aberto = float(div_master.loc[~master_resolved, "VALOR"].sum()) if total_itens else 0.0
    pct_res = (itens_res / total_itens * 100.0) if total_itens else 0.0
...
""",
    ...
    # ========================
    # Resumo para priorização (sem sobreposição)
    # ========================
    with st.expander("Resumo para priorização (abertos, top impacto, pendências)", expanded=True):
        # Base: apenas em aberto
        abertos = div_master.loc[~master_resolved].copy()
        abertos["DATA"] = pd.to_datetime(abertos["DATA"], errors="coerce")
        abertos["ABS_VALOR"] = abertos["VALOR"].abs()

        top10 = abertos.sort_values("ABS_VALOR", ascending=False).head(10).copy()
        top10_view = top10[["ORIGEM_EXIBICAO","DATA","DOCUMENTO","VALOR","NUCLEO_CONFIRMADO"]].copy()
        top10_view["DATA"] = top10_view["DATA"].dt.strftime("%d/%m/%Y").fillna("")

        # Pendências: sem núcleo (na prática sempre tem, mas deixo aqui se você quiser reativar depois)
        sem_nucleo = int((abertos["NUCLEO_CONFIRMADO"].astype(str).str.strip() == "").sum())

        # Distribuição por núcleo (abertos)
        dist = abertos.groupby("NUCLEO_CONFIRMADO", dropna=False).agg(
            Itens=("VALOR", "size"),
            Valor=("VALOR", "sum")
        ).reset_index().sort_values("Valor", ascending=False)

        r1, r2 = st.columns([2.1, 1.2])
        with r1:
            st.markdown("**Top 10 em aberto por impacto**")
            st.dataframe(top10_view, use_container_width=True, height=260)

        with r2:
            st.markdown("**Pendências**")
            st.write(f"Itens em aberto sem Núcleo confirmado: **{sem_nucleo}**")
            st.write("Atalhos:")
            cqa, cqb = st.columns(2)
            with cqa:
                if st.button("Ver somente Crítica", use_container_width=True):
                    st.session_state["_quick"] = "Somente Crítica"
                    st.rerun()
            with cqb:
                if st.button("Ver Atenção + Crítica", use_container_width=True):
                    st.session_state["_quick"] = "Atenção + Crítica"
                    st.rerun()

            st.markdown("**Distribuição por Núcleo (abertos)**")
            st.dataframe(dist, use_container_width=True, height=180)

    # aplicar atalho por botão (se existir)
    if st.session_state.get("_quick"):
        val = st.session_state["_quick"]
        st.session_state["_quick"] = None
        st.info(f"Atalho aplicado: {val}. Ajuste no seletor 'Atalho' se desejar manter.")
        # não forço o selectbox pra não brigar com estado; só aviso

    # ========================
    # Ações em massa (UX mais direto)
    # ========================
    st.markdown("### Ações em massa")
    st.markdown('<div class="cm-help">Dica: use o filtro e aplique ação no que está na tela (ou por IDs).</div>', unsafe_allow_html=True)

    scope = st.radio("Aplicar em:", ["Itens filtrados (tela atual)", "IDs informados"], horizontal=True)

    # IDs que estão na tela (df é a tabela filtrada)
    df_view_ids = list(df.index)

    if scope == "Itens filtrados (tela atual)":
        target_ids = df_view_ids
        st.write(f"Selecionados: **{len(target_ids)}** itens (do filtro atual).")
    else:
        ...
        st.write(f"Selecionados: **{len(target_ids)}** itens (por IDs).")
    ...
    with cD:
        bulk_obs = st.text_input("Obs (aplicar / sobrescrever)", value="")
    ...
    if st.button("Aplicar nos selecionados", type="primary", disabled=(len(target_ids) == 0)):
        ...
        if bulk_nucleo != "(não alterar)":
            ...
            dm.loc[target_ids, "MOTIVO_CONFIRMADO"] = NUCLEO_MOTIVO.get(bulk_nucleo, NUCLEO_MOTIVO["Não identificado"])
    ...
    st.markdown('<div class="cm-help">Regra: para marcar como Resolvido, Núcleo deve estar preenchido.</div>', unsafe_allow_html=True)

    # Colunas de tela: ORIGEM_EXIBICAO com sinal embutido
    view_cols = [
        "ORIGEM_EXIBICAO", "DATA", "DOCUMENTO", "HISTORICO_OPERACAO", "CHAVE_DOC", "VALOR",
    ...
    column_config = {
        "ORIGEM_EXIBICAO": st.column_config.TextColumn("ORIGEM", disabled=True),
    ...
    if edited is not None and len(edited) == len(df_view_display):
        ...
        # Reaplicar motivo automaticamente quando mudar núcleo
        if "NUCLEO_CONFIRMADO" in to_update.columns:
            new_nuc = to_update["NUCLEO_CONFIRMADO"].astype(str)
            dm.loc[to_update.index, "NUCLEO_CONFIRMADO"] = new_nuc.values
            dm.loc[to_update.index, "MOTIVO_CONFIRMADO"] = new_nuc.map(lambda x: NUCLEO_MOTIVO.get(x, NUCLEO_MOTIVO["Não identificado"])).values

        # Status e Obs
        dm.loc[to_update.index, "STATUS"] = to_update["STATUS"].values
        dm.loc[to_update.index, "OBS_USUARIO"] = to_update["OBS_USUARIO"].values

        # Coerência Resolvido
        res_col = to_update["RESOLVIDO"].fillna(False)
        dm.loc[to_update.index, "RESOLVIDO"] = res_col.values
        dm.loc[to_update.index[res_col], "STATUS"] = "Resolvido"

        # Regra: se RESOLVIDO=True, Núcleo obrigatório
        nuc = dm.loc[to_update.index, "NUCLEO_CONFIRMADO"].astype(str).str.strip()
        bad = res_col & (nuc.eq("") | nuc.isna())
        if bad.any():
            st.error("Para marcar como Resolvido, é obrigatório informar o Núcleo confirmado. Os itens inválidos foram desmarcados.")
            dm.loc[to_update.index[bad], "RESOLVIDO"] = False
            dm.loc[to_update.index[bad], "STATUS"] = "Pendente"
    ...
    if pick_id in div_master.index:
        ...
        resumo = (
            f"ID: {pick_id}\n"
            f"ORIGEM: {r.get('ORIGEM_EXIBICAO','')}\n"
            f"DATA: {dt_txt}\n"
...
<div class="cm-detail">
  ...
  <div class="row"><span class="label">Origem:</span> <span class="val">{r.get('ORIGEM_EXIBICAO','')}</span></div>
...
""",
    ...
    # Exporta exatamente o que está na tela (df_view é filtrado)
    excel_bytes = to_excel_divergencias_filtradas(
        df_filtrado=df_view,
        total_filtrado=float(df_view["VALOR"].sum()) if len(df_view) else 0.0,
        total_aberto=float(div_master.loc[~(div_master["RESOLVIDO"] | (div_master["STATUS"].str.lower().eq("resolvido"))), "VALOR"].sum()) if len(div_master) else 0.0,
        filtros=filtros,
        stats=stats,
