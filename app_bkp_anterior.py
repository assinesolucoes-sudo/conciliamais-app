...
MATCH_RULES_FILE = "matching_rules.json"
...
def render_cruzamento_inteligente_v2():
    ...
    def _clean_report_label(label, nome_base_a=None, nome_base_b=None):
        label = str(label)
        for prefix in ["TOTALIZADOR_", "CHAVE_", "VALOR_", "DIF_"]:
            if label.startswith(prefix):
                label = label[len(prefix):]
        if nome_base_a:
            label = label.replace(f"{nome_base_a}_", "")
            label = label.replace(f"{nome_base_a}__", "")
        if nome_base_b:
            label = label.replace(f"{nome_base_b}_", "")
            label = label.replace(f"{nome_base_b}__", "")
        label = label.replace("__", " | ").replace("_", " ")
        return str(label).strip()

    def _safe_group_sort(df, cols):
        try:
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols):
        if df_result is None or df_result.empty or not totalizador_cols:
            return pd.DataFrame()
        group_cols = [c for c in totalizador_cols if c in df_result.columns]
        if not group_cols:
            return pd.DataFrame()
        work = df_result.copy()
        diff_cols = [m.get("diff_col") for m in compare_meta if m.get("diff_col") in work.columns]
        if diff_cols:
            work["__TEM_DIF__"] = False
            for dc in diff_cols:
                work[dc] = pd.to_numeric(work[dc], errors="coerce").fillna(0.0).round(2)
                work["__TEM_DIF__"] = work["__TEM_DIF__"] | work[dc].ne(0)
        else:
            work["__TEM_DIF__"] = work["RESULTADO_FINAL"].astype(str).eq("Match com divergência")
        work = work[work["__TEM_DIF__"]].copy()
        if work.empty:
            return pd.DataFrame()
        agg_dict = {
            "Quantidade de Registros": (group_cols[0], "size"),
        }
        agg = work.groupby(group_cols, dropna=False).agg(**agg_dict).reset_index()
        for meta in compare_meta:
            diff_col = meta.get("diff_col")
            diff_label = meta.get("diff_label", _clean_report_label(diff_col))
            if diff_col in work.columns:
                grp = work.groupby(group_cols, dropna=False)[diff_col].sum().reset_index()
                grp = grp.rename(columns={diff_col: f"Diferença {diff_label}"})
                agg = agg.merge(grp, on=group_cols, how="left")
        num_cols = [c for c in agg.columns if c.startswith("Diferença ")]
        if num_cols:
            mask = agg[num_cols].fillna(0).abs().sum(axis=1).round(2).ne(0)
            agg = agg[mask].copy()
        return _safe_group_sort(agg, group_cols)

    def _build_exec_summary(df_result, resumo_dict, compare_meta, totalizador_df):
        payload = {
            "indicadores": [
            ...
            "impacto": [],
            "detalhe": totalizador_df if totalizador_df is not None else pd.DataFrame(),
        }
        ...
        for meta in compare_meta:
            diff_col = meta.get("diff_col")
            diff_label = meta.get("diff_label", _clean_report_label(diff_col))
            if diff_col in df_result.columns:
                serie = pd.to_numeric(df_result[diff_col], errors="coerce").fillna(0.0).round(2)
                total_diff = float(serie.sum())
                qtd = int(serie.ne(0).sum())
                if qtd > 0 and round(total_diff, 2) != 0:
                    payload["impacto"].append((f"{diff_label} com diferença", total_diff, qtd))
                elif qtd > 0:
                    payload["impacto"].append((f"{diff_label} com diferença", total_diff, qtd))
        return payload

    def _to_excel_package(df_result, resumo_dict, text_priority_cols=None, totalizador_df=None, summary_payload=None):
        output = BytesIO()
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            wb = writer.book
            fmt_hdr = wb.add_format({"bold": True, "bg_color": "#DBEAFE", "border": 1, "align": "center", "valign": "vcenter"})
            fmt_section = wb.add_format({"bold": True, "bg_color": "#E0F2FE", "border": 1, "align": "left", "valign": "vcenter"})
            ...
            if summary_payload is None:
                summary_payload = _build_exec_summary(df_result, resumo_dict, [], totalizador_df)
            wsr = wb.add_worksheet("RESUMO_EXECUTIVO")
            writer.sheets["RESUMO_EXECUTIVO"] = wsr
            wsr.set_column(0, 0, 34)
            wsr.set_column(1, 1, 20)
            wsr.set_column(3, 4, 18)
            wsr.set_column(6, 20, 22)
            row = 0
            wsr.write(row, 0, "Indicador", fmt_hdr)
            wsr.write(row, 1, "Valor", fmt_hdr)
            for item in summary_payload.get("indicadores", []):
                row += 1
                wsr.write(row, 0, item[0], fmt_label)
                wsr.write(row, 1, item[1], fmt_value)
            wsr.write(1, 3, "Visão Executiva", fmt_hdr)
            for i, (lbl, val) in enumerate(summary_payload.get("visao", []), start=2):
                wsr.write(i, 3, lbl, fmt_label)
                wsr.write(i, 4, val, fmt_kpi if isinstance(val, (int, float, np.integer, np.floating)) else fmt_value)
            row = max(row + 2, 9)
            wsr.write(row, 0, "Impacto financeiro das diferenças", fmt_section)
            row += 1
            wsr.write(row, 0, "Indicador", fmt_hdr)
            wsr.write(row, 1, "Valor", fmt_hdr)
            wsr.write(row, 2, "Qtd. registros", fmt_hdr)
            impactos = summary_payload.get("impacto", [])
            if impactos:
                for titulo, valor, qtd in impactos:
                    row += 1
                    wsr.write(row, 0, titulo, fmt_label)
                    wsr.write_number(row, 1, float(round(valor, 2)), fmt_num)
                    wsr.write_number(row, 2, int(qtd), fmt_int)
            else:
                row += 1
                wsr.write(row, 0, "Sem diferenças financeiras para exibir", fmt_value)
            detalhe = summary_payload.get("detalhe")
            if detalhe is not None and not detalhe.empty:
                start_row = row + 3
                wsr.write(start_row, 0, "Onde estão as diferenças", fmt_section)
                start_row += 1
                for c, col in enumerate(detalhe.columns):
                    wsr.write(start_row, c, col, fmt_hdr)
                for r in range(len(detalhe)):
                    for c, col in enumerate(detalhe.columns):
                        val = detalhe.iloc[r, c]
                        if isinstance(val, (int, float, np.integer, np.floating)) and "Diferença " in str(col):
                            wsr.write_number(start_row + 1 + r, c, float(round(val, 2)), fmt_num)
                        else:
                            wsr.write(start_row + 1 + r, c, val, fmt_value)
    ...
    with c1:
        ...
        st.markdown(f"**{nome_base_a or 'Base A'} — referência**")
    ...
    with c2:
        ...
        st.markdown(f"**{nome_base_b or 'Base B'} — base a validar / confrontar**")
    ...
    direcao = st.radio("Como deseja comparar as bases?", [
        f"Validar {nome_base_a} contra {nome_base_b}",
        f"Validar {nome_base_b} contra {nome_base_a}",
    ...
    st.caption("Use esta etapa quando quiser traduzir descrições da base de referência para um valor correspondente da outra base antes do match principal.")
    assoc_on = st.checkbox("Quero criar uma regra de associação entre campos", value=False, key="v6_assoc_on")
    association_specs = []
    if assoc_on:
        a1, a2 = st.columns(2)
        with a1:
            assoc_col_a = st.selectbox(f"Campo descritivo da {nome_base_a}", list(df_a.columns), key="v6_assoc_col_a")
        with a2:
            assoc_col_b = st.selectbox(f"Campo de destino da {nome_base_b}", list(df_b.columns), key="v6_assoc_col_b")
        uniq_vals = sorted([x for x in _force_text_series(df_a[assoc_col_a]).dropna().unique().tolist() if str(x).strip() != ""])
        uniq_targets_b = sorted([x for x in _force_text_series(df_b[assoc_col_b]).dropna().unique().tolist() if str(x).strip() != ""])
        default_rows = [{"USAR": False, f"VALOR_{nome_base_a}": v, f"VALOR_{nome_base_b}": ""} for v in uniq_vals[:300]]
        assoc_df = st.data_editor(
            ...
            column_config={
                ...
                f"VALOR_{nome_base_b}": st.column_config.SelectboxColumn(nome_base_b, options=uniq_targets_b),
            },
            key="v6_assoc_grid",
        )
        assoc_df = assoc_df[(assoc_df["USAR"] == True) & (assoc_df[f"VALOR_{nome_base_b}"].astype(str).str.strip() != "")].copy()
        if not assoc_df.empty:
            ...
            association_specs.append({
                ...
                "map_col_a": map_col_a,
                "mapping": dict(zip(assoc_df[f"VALOR_{nome_base_a}"].astype(str), assoc_df[f"VALOR_{nome_base_b}"].astype(str))),
            })
            st.success(f"{len(assoc_df)} regra(s) de associação pronta(s) para o processamento.")
    ...
    for _, key_row in selected_keys.iterrows():
        display = _clean_report_label(key_row["CAMPO_BASE_A"])
        totalizador_meta[display] = ("A", key_row["CAMPO_BASE_A"], key_row["CAMPO_BASE_B"])
    for spec in association_specs:
        display = _clean_report_label(spec["source_col_a"])
        totalizador_meta[display] = ("MAP", spec["map_col_a"], spec["target_col_b"])
    if not totalizador_meta:
        for _, cp in selected_compares.iterrows():
            display = _clean_report_label(cp["CAMPO_BASE_A"])
            totalizador_meta[display] = ("A", cp["CAMPO_BASE_A"], cp["CAMPO_BASE_B"])
    totalizador_choices = st.multiselect(
        "Campos para totalização no resumo executivo",
        list(totalizador_meta.keys()),
        default=list(totalizador_meta.keys())[:1],
        help="Selecione apenas os campos-chave ou campos de associação que fazem sentido para o resumo executivo.",
    )
    ...
    with st.spinner("Processando análise..."):
        ...
        def _process_direction(origin_label):
            if origin_label == "A":
                left = base_a.copy()
                right = base_b_pref.copy()
                base_validada = nome_base_a
                base_ref = nome_base_b
            else:
                left = base_b.copy()
                right = base_a_pref.copy()
                base_validada = nome_base_b
                base_ref = nome_base_a
            merged = left.merge(right, on="__KEY__", how="left")
            ...
            result["__A_COUNT__"] = merged["__KEY__"].map(a_counts).fillna(0).astype(int)
            result["__B_COUNT__"] = merged["__KEY__"].map(b_counts).fillna(0).astype(int)
            cen_res = result.apply(lambda r: _classify_row(r["__A_COUNT__"], r["__B_COUNT__"], origin_label), axis=1, result_type="expand")
            result["CENARIO_MATCH"] = cen_res[0]
            result["RESULTADO_FINAL"] = cen_res[1]
            result["STATUS_MATCH"] = result["CENARIO_MATCH"]
            compare_meta = []
            ...
            for _, key_row in selected_keys.iterrows():
                ...
                result[ca] = merged.get(f"A__{key_row['CAMPO_BASE_A']}", merged.get(key_row['CAMPO_BASE_A'], "")).fillna("") if origin_label == "B" else merged.get(key_row['CAMPO_BASE_A'], "").fillna("")
                result[cb] = merged.get(f"B__{key_row['CAMPO_BASE_B']}", merged.get(key_row['CAMPO_BASE_B'], "")).fillna("") if origin_label == "A" else merged.get(key_row['CAMPO_BASE_B'], "").fillna("")
            totalizador_result_cols = []
            for choice in totalizador_choices:
                lado, campo_a, campo_b = totalizador_meta.get(choice, ("A", "", ""))
                res_col = choice
                if lado == "MAP":
                    result[res_col] = merged.get(campo_a, pd.Series("", index=merged.index)).fillna("")
                elif origin_label == "A":
                    result[res_col] = merged.get(campo_a, pd.Series("", index=merged.index)).fillna("")
                else:
                    result[res_col] = merged.get(f"A__{campo_a}", pd.Series("", index=merged.index)).fillna("")
                totalizador_result_cols.append(res_col)
            for _, cp in selected_compares.iterrows():
                if origin_label == "A":
                    sa = merged.get(cp["CAMPO_BASE_A"], pd.Series("", index=merged.index))
                    ...
                    sb = merged.get(cp["CAMPO_BASE_B"], pd.Series("", index=merged.index))
                raw_a, raw_b, diff, status = _compare_series(sa, sb, mode=cp["TIPO"], tol=float(cp["TOLERANCIA"] or 0.0))
                label_a = _clean_report_label(cp["CAMPO_BASE_A"])
                label_b = _clean_report_label(cp["CAMPO_BASE_B"])
                diff_label = label_a if label_a == label_b else f"{label_a} x {label_b}"
                c_va = f"{label_a} ({nome_base_a})"
                c_vb = f"{label_b} ({nome_base_b})"
                c_df = f"Diferença {diff_label}"
                result[c_va] = raw_a
                result[c_vb] = raw_b
                result[c_df] = diff.round(2) if pd.api.types.is_numeric_dtype(diff) else diff
                has_div = has_div | (result["CENARIO_MATCH"].eq("Encontrado") & status.eq("Divergente"))
                compare_meta.append({"diff_col": c_df, "diff_label": diff_label})
            result.loc[result["CENARIO_MATCH"].eq("Encontrado") & has_div, "RESULTADO_FINAL"] = "Match com divergência"
            result = result.drop(columns=["__A_COUNT__", "__B_COUNT__"], errors="ignore")
            return result, compare_meta, totalizador_result_cols

        if direcao == f"Validar {nome_base_a} contra {nome_base_b}":
            df_result, compare_meta, totalizador_result_cols = _process_direction("A")
        elif direcao == f"Validar {nome_base_b} contra {nome_base_a}":
            df_result, compare_meta, totalizador_result_cols = _process_direction("B")
        else:
            df_r1, compare_meta, totalizador_result_cols = _process_direction("A")
            df_r2, compare_meta_b, totalizador_result_cols_b = _process_direction("B")
            compare_meta = compare_meta + compare_meta_b
            totalizador_result_cols = list(dict.fromkeys(totalizador_result_cols + totalizador_result_cols_b))
        ...
        for _, cp in selected_compares.iterrows():
            ...
            label_b = _clean_report_label(cp["CAMPO_BASE_B"])
            diff_label = label_a if label_a == label_b else f"{label_a} x {label_b}"
            cols_cmp = [f"{label_a} ({nome_base_a})", f"{label_b} ({nome_base_b})", f"Diferença {diff_label}"]
            for c in cols_cmp:
        ...
        totalizador_df = _build_totalizador(df_result, compare_meta, totalizador_result_cols) if gerar_resumo_exec else pd.DataFrame()
        text_priority_cols = [c for c in df_result.columns if any(x in str(c).upper() for x in ["CHAVE", "COD", "CÓD", "FILIAL", "LOJA", "PATRIM", "PLAQUETA"])]
        summary_payload = _build_exec_summary(df_result, resumo_dict, compare_meta, totalizador_df)
        excel_bytes = _to_excel_package(df_result=df_result, resumo_dict=resumo_dict, text_priority_cols=text_priority_cols, totalizador_df=totalizador_df, summary_payload=summary_payload)
