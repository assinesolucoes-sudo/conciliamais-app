...
def render_cruzamento_inteligente_v2():
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols):
        ...
        work = df_result.copy()
        agg_dict = {
            "QTDE_REGISTROS": (group_cols[0], "size"),
            "QTDE_ENCONTRADOS": ("CENARIO_MATCH", lambda s: int((s.astype(str) == "Encontrado").sum())),
            "QTDE_DIVERGENCIAS": ("RESULTADO_FINAL", lambda s: int((s.astype(str) == "Match com divergência").sum())),
            "QTDE_NAO_ENCONTRADOS": ("CENARIO_MATCH", lambda s: int((s.astype(str).str.contains("Não encontrado", na=False)).sum())),
            "QTDE_DUPLICIDADES": ("CENARIO_MATCH", lambda s: int((s.astype(str).str.contains("Duplicidade", na=False)).sum())),
        }
        ...
        for meta in compare_meta:
            val_a_col = meta.get("val_a_col")
            val_b_col = meta.get("val_b_col")
            diff_col = meta.get("diff_col")
            label = meta.get("label") or _label_exec(diff_col)
            if val_a_col in work.columns:
                grp_a = work.assign(__TMP=pd.to_numeric(work[val_a_col], errors="coerce").fillna(0.0).round(2)).groupby(group_cols, dropna=False)["__TMP"].sum().reset_index().rename(columns={"__TMP": f"{label} - Base A"})
                agg = agg.merge(grp_a, on=group_cols, how="left")
            if val_b_col in work.columns:
                grp_b = work.assign(__TMP=pd.to_numeric(work[val_b_col], errors="coerce").fillna(0.0).round(2)).groupby(group_cols, dropna=False)["__TMP"].sum().reset_index().rename(columns={"__TMP": f"{label} - Base B"})
                agg = agg.merge(grp_b, on=group_cols, how="left")
            if diff_col in work.columns:
                diff_numeric = pd.to_numeric(work[diff_col], errors="coerce").fillna(0.0).round(2)
                grp_d = work.assign(__TMP=diff_numeric).groupby(group_cols, dropna=False)["__TMP"].sum().reset_index().rename(columns={"__TMP": f"Diferença {label}"})
                agg = agg.merge(grp_d, on=group_cols, how="left")
                total_diff = float(diff_numeric.sum())
                if round(total_diff, 2) != 0:
                    financial_kpis.append((f"Diferença total {label}", round(total_diff, 2)))
                diff_sum_cols.append(f"Diferença {label}")

        rename_map = {c: _label_exec(c) for c in group_cols}
        agg = agg.rename(columns=rename_map)
        if diff_sum_cols:
            diff_sum_cols = [c for c in diff_sum_cols if c in agg.columns]
            if diff_sum_cols:
                mask_non_zero = agg[diff_sum_cols].fillna(0).abs().sum(axis=1).round(2) != 0
                agg = agg[mask_non_zero].copy()
        if "QTDE_DIVERGENCIAS" in agg.columns:
            agg = agg[(agg["QTDE_DIVERGENCIAS"] > 0) | (agg.get("QTDE_NAO_ENCONTRADOS", 0) > 0) | (agg.get("QTDE_DUPLICIDADES", 0) > 0)].copy()
        if not agg.empty:
            sort_cols = [rename_map[c] for c in group_cols if c in rename_map]
    ...
    def _to_excel_package(df_result, resumo_dict, text_priority_cols=None, totalizador_df=None, financial_kpis=None):
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            if financial_kpis:
                wsr.write("D9", "Impacto financeiro", fmt_hdr)
                row_fin = 9
                for titulo, valor in financial_kpis:
                    wsr.write(row_fin, 3, titulo, fmt_label)
                    wsr.write_number(row_fin, 4, float(round(valor, 2)), fmt_num)
                    row_fin += 1
            if totalizador_df is not None and not totalizador_df.empty:
                start_row = 1
                start_col = 6
                wsr.write(0, start_col, "Resumo executivo por chave", fmt_hdr)
                for c, col in enumerate(totalizador_df.columns):
                    wsr.write(start_row, start_col + c, col, fmt_hdr)
                    width = min(max(len(str(col)) + 4, 14), 34)
                    wsr.set_column(start_col + c, start_col + c, width)
                for r in range(len(totalizador_df)):
                    for c, col in enumerate(totalizador_df.columns):
                        val = totalizador_df.iloc[r, c]
                        if isinstance(val, (int, float, np.integer, np.floating)) and ("Diferença" in str(col) or "Base A" in str(col) or "Base B" in str(col)):
                            wsr.write_number(r + start_row + 1, start_col + c, float(round(val, 2)), fmt_num)
                        else:
                            wsr.write(r + start_row + 1, start_col + c, val, fmt_value)
    ...
    if assoc_on:
        ...
        current_grid = st.session_state.get(grid_key)
        if current_grid is None or not isinstance(current_grid, pd.DataFrame) or current_grid.empty:
    ...
    totalizador_map = {}
    for c in work_a.columns:
        lbl = f"{nome_base_a} :: {c}"
        totalizador_map[lbl] = (nome_base_a, c)
    for c in work_b.columns:
        lbl = f"{nome_base_b} :: {c}"
        totalizador_map[lbl] = (nome_base_b, c)
    for spec in association_specs:
        map_col = spec.get("map_col_a")
        src = spec.get("source_col_a")
        dst = spec.get("target_col_b")
        if map_col:
            lbl = f"Associação :: {src} → {dst}"
            totalizador_map[lbl] = (nome_base_a, map_col)
    totalizador_options = list(totalizador_map.keys())
    default_totalizadores = []
    totalizador_choices = st.multiselect(
        "Campos para totalização no resumo executivo",
        totalizador_options,
        default=default_totalizadores,
        help="Selecione os campos da Base A, da Base B e também os campos gerados pelas associações confirmadas.",
        key="v8_totalizadores"
    )
        ...
        def _process_direction(origin_label):
            ...
            for choice in totalizador_choices:
                if choice not in totalizador_map:
                    continue
                lado, campo = totalizador_map[choice]
                pref = "A__" if lado == nome_base_a else "B__"
                nome_limpo = re.sub(r"[^A-Za-z0-9_À-ÿ]+", "_", str(campo)).strip("_")
                label_limpo = choice.replace(" :: ", " - ").replace("Associação", "Associacao")
                res_col = f"TOTALIZADOR__{label_limpo}"
                if origin_label == "A" and lado == nome_base_a:
                    result[res_col] = merged.get(campo, pd.Series("", index=merged.index)).fillna("")
                elif origin_label == "B" and lado == nome_base_b:
                    result[res_col] = merged.get(campo, pd.Series("", index=merged.index)).fillna("")
                else:
                    result[res_col] = merged.get(f"{pref}{campo}", pd.Series("", index=merged.index)).fillna("")
            ...
            for _, cp in selected_compares.iterrows():
                ...
                c_va = f"VALOR_{nome_base_a}_{cp['CAMPO_BASE_A']}" if origin_label == "A" else f"VALOR_{nome_base_a}_{cp['CAMPO_BASE_A']}"
                c_vb = f"VALOR_{nome_base_b}_{cp['CAMPO_BASE_B']}" if origin_label == "A" else f"VALOR_{nome_base_b}_{cp['CAMPO_BASE_B']}"
