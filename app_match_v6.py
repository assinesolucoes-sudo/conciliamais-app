...
def render_cruzamento_inteligente_v2():
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols):
        ...
        for meta in compare_meta:
            ...
            if diff_col in work.columns:
                tmp = pd.to_numeric(work[diff_col], errors="coerce").fillna(0.0).round(2)
                grp = work.assign(__TMP=tmp).groupby(group_cols, dropna=False)["__TMP"].sum().reset_index()
                grp = grp.rename(columns={"__TMP": f"SOMA_{diff_col}"})
                ...
                for r in range(len(totalizador_df)):
                    for c, col in enumerate(totalizador_df.columns):
                        ...
                        if isinstance(val, (int, float, np.integer, np.floating)) and ("SOMA_" in str(col).upper()):
                            wsr.write_number(r + 2, start_col + c, float(round(val, 2)), fmt_num)
                        else:
    ...
    if assoc_on:
        ...
        default_rows = [{"USAR": False, f"VALOR_{nome_base_a}": v, f"VALOR_{nome_base_b}": ""} for v in uniq_vals[:200]]
        assoc_df = st.data_editor(
            ...
            column_config={
                ...
                f"VALOR_{nome_base_b}": st.column_config.TextColumn(nome_base_b),
            },
            key="v6_assoc_grid",
        )
    ...
    totalizador_options = [f"{nome_base_a} :: {c}" for c in work_a.columns] + [f"{nome_base_b} :: {c}" for c in work_b.columns]
    totalizador_choices = st.multiselect(
        "Campos para totalização no resumo executivo",
        totalizador_options,
        default=[],
        help="Você pode escolher um ou vários campos, como filial e grupo.",
    )
    ...
    with st.spinner("Processando análise..."):
        ...
        def _process_direction(origin_label):
            ...
            for choice in totalizador_choices:
                lado, campo = choice.split(" :: ", 1)
                pref = "A__" if lado == nome_base_a else "B__"
                res_col = f"TOTALIZADOR_{lado}_{campo}"
                if origin_label == "A" and lado == nome_base_a:
                    result[res_col] = merged.get(campo, "").fillna("")
                elif origin_label == "B" and lado == nome_base_b:
                    result[res_col] = merged.get(campo, "").fillna("")
                else:
                    result[res_col] = merged.get(f"{pref}{campo}", "").fillna("")
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V6_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
