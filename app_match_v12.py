...
def render_cruzamento_inteligente_v2():
    ...
    # Campos-chave escolhidos no match: entram pré-selecionados no resumo executivo.
    ...
    # Campos das regras de associação: disponibiliza a visão de origem e também do destino associado.
    for spec in association_specs:
    ...
    # Demais campos das bases continuam disponíveis para inclusão manual quando necessário.
    ...
    for _, key_row in selected_keys.iterrows():
        lbl = _canon_totalizer_label(key_row["CAMPO_BASE_A"], key_row["CAMPO_BASE_B"])
        if lbl in totalizador_options and lbl not in default_totalizadores:
            default_totalizadores.append(lbl)

    totalizador_choices = st.multiselect(
        "Campos para totalização no resumo executivo",
        totalizador_options,
        default=default_totalizadores,
        help="Os campos-chave escolhidos no match já vêm pré-selecionados. Você pode remover ou adicionar outros agrupadores para o resumo executivo.",
        key="v12_totalizadores"
    )
    ...
    with st.spinner("Processando análise..."):
        ...
        # Auditoria completa em visão única: evita duplicar linhas ao processar dois sentidos.
        # O merge outer já considera itens sem par em qualquer uma das bases.
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V12_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
