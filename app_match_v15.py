...
def render_cruzamento_inteligente_v2():
        ...
        for r in range(len(df)):
            for c, col in enumerate(df.columns):
                ...
                if col in text_priority_cols or _excel_col_type(col) == "text":
                    ...
                    ws.write_string(r + 1, c, str(val), fmt_text)
        for c, col in enumerate(df.columns):
            sample = [str(col)] + df.iloc[:200, c].astype(str).tolist()
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols, raw_base_a=None, raw_base_b=None, selected_compares_df=None, group_specs=None):
        if selected_compares_df is None or selected_compares_df.empty:
            return pd.DataFrame(), []

        selected_meta = [m for m in compare_meta if m.get("selected", True)] or list(compare_meta)
        if not selected_meta:
            return pd.DataFrame(), []

        compares_lookup = {}
        for _, cp in selected_compares_df.iterrows():
            compares_lookup[str(cp.get("CAMPO_BASE_A", "")).strip()] = cp

        # Fechamento financeiro global sempre sobre as bases completas.
        financial_kpis = []
        total_bridge = []
        for meta in selected_meta:
            label = str(meta.get("label") or _label_exec(meta.get("diff_col"))).strip()
            cp = compares_lookup.get(label)
            if cp is None:
                continue
            va_all = _round_money_series(raw_base_a.get(cp["CAMPO_BASE_A"], pd.Series(dtype=object))).sum() if raw_base_a is not None else 0.0
            vb_all = _round_money_series(raw_base_b.get(cp["CAMPO_BASE_B"], pd.Series(dtype=object))).sum() if raw_base_b is not None else 0.0
            ...
            total_bridge.append((label, vd_all))

        # Campos para agrupar: usar specs explícitas quando disponíveis.
        if group_specs is None:
            group_specs = []
            for col in totalizador_cols:
                group_specs.append({"label": _label_exec(col), "campo_a": None, "campo_b": None, "result_col": col})

        if not group_specs:
            return pd.DataFrame(), financial_kpis

        # Agregado por chave diretamente das bases completas, para fechar com o total oficial.
        a = raw_base_a.copy() if raw_base_a is not None else pd.DataFrame()
        b = raw_base_b.copy() if raw_base_b is not None else pd.DataFrame()
        for spec in group_specs:
            lbl = spec["label"]
            ca = spec.get("campo_a")
            cb = spec.get("campo_b")
            a[lbl] = _force_text_series(a[ca]) if ca and ca in a.columns else ""
            b[lbl] = _force_text_series(b[cb]) if cb and cb in b.columns else ""

        group_labels = [spec["label"] for spec in group_specs]
        a["__COUNT_A__"] = 1
        b["__COUNT_B__"] = 1
        agg_a = a.groupby(group_labels, dropna=False, sort=True).agg({"__COUNT_A__": "sum"}).reset_index()
        agg_b = b.groupby(group_labels, dropna=False, sort=True).agg({"__COUNT_B__": "sum"}).reset_index()
        ...
        for meta in selected_meta:
            ...
            if cp is None:
                continue
            col_a = cp["CAMPO_BASE_A"]
            col_b = cp["CAMPO_BASE_B"]
            a_num = _round_money_series(a.get(col_a, pd.Series(dtype=object)))
            b_num = _round_money_series(b.get(col_b, pd.Series(dtype=object)))
            agg_tmp_a = pd.concat([a[group_labels], a_num.rename(f"{label} {nome_base_a}")], axis=1).groupby(group_labels, dropna=False, sort=True).sum(numeric_only=True).reset_index()
            agg_tmp_b = pd.concat([b[group_labels], b_num.rename(f"{label} {nome_base_b}")], axis=1).groupby(group_labels, dropna=False, sort=True).sum(numeric_only=True).reset_index()
            agg_a = agg_a.merge(agg_tmp_a, on=group_labels, how="left")
            agg_b = agg_b.merge(agg_tmp_b, on=group_labels, how="left")

        agg = agg_a.merge(agg_b, on=group_labels, how="outer")
        for c in agg.columns:
            if c not in group_labels:
                agg[c] = pd.to_numeric(agg[c], errors="coerce").fillna(0)

        # Indicadores de divergência/motivo vindos do resultado do match.
        if df_result is not None and not df_result.empty:
            ...
            for spec in group_specs:
                ...
                if result_col and result_col in work.columns:
                    work[lbl] = _force_text_series(work[result_col])
                elif lbl not in work.columns:
                    work[lbl] = ""
            grp_res = work.groupby(group_labels, dropna=False, sort=True)
            aux_rows = []
            for keys, grp in grp_res:
                if len(group_labels) == 1:
                    keys = (keys,)
                row = {group_labels[i]: keys[i] for i in range(len(group_labels))}
                row["Qtde Divergências"] = int((grp.get("RESULTADO_FINAL", pd.Series([], dtype=object)).astype(str) == "Match com divergência").sum())
                row["Qtde Ausentes"] = int(grp.get("CENARIO_MATCH", pd.Series([], dtype=object)).astype(str).str.contains("Não encontrado", na=False).sum())
                row["Qtde Duplicidades"] = int(grp.get("CENARIO_MATCH", pd.Series([], dtype=object)).astype(str).str.contains("Duplicidade", na=False).sum())
                aux_rows.append(row)
            aux_df = pd.DataFrame(aux_rows)
            if not aux_df.empty:
                agg = agg.merge(aux_df, on=group_labels, how="left")

        agg["Qtde {}".format(nome_base_a)] = agg.pop("__COUNT_A__") if "__COUNT_A__" in agg.columns else 0
        agg["Qtde {}".format(nome_base_b)] = agg.pop("__COUNT_B__") if "__COUNT_B__" in agg.columns else 0
        for c in ["Qtde Divergências", "Qtde Ausentes", "Qtde Duplicidades"]:
        ...
        for meta in selected_meta:
            label = str(meta.get("label") or _label_exec(meta.get("diff_col"))).strip()
            ca = f"{label} {nome_base_a}"
            cb = f"{label} {nome_base_b}"
            if ca not in agg.columns:
                agg[ca] = 0.0
            if cb not in agg.columns:
                agg[cb] = 0.0
            cd = f"Diferença {label}"
            agg[cd] = (pd.to_numeric(agg[ca], errors="coerce").fillna(0) - pd.to_numeric(agg[cb], errors="coerce").fillna(0)).round(2)
            diff_cols.append(cd)

        # Mantém apenas linhas relevantes para conciliação.
        focus_cols = [f"Qtde {nome_base_a}", f"Qtde {nome_base_b}", "Qtde Divergências", "Qtde Ausentes", "Qtde Duplicidades"] + diff_cols
        mask_keep = agg[focus_cols].apply(pd.to_numeric, errors="coerce").fillna(0).abs().sum(axis=1) > 0
        if mask_keep.any():
            agg = agg[mask_keep].copy()
        agg = agg.sort_values(group_labels).reset_index(drop=True)

        # Ponte de conciliação por campo: o total do resumo precisa fechar com o global.
        for label, global_diff in total_bridge:
            resumo_diff = float(pd.to_numeric(agg.get(f"Diferença {label}"), errors="coerce").fillna(0).sum()) if not agg.empty else 0.0
            if round(global_diff - resumo_diff, 2) != 0:
                financial_kpis.append((f"Diferença não explicada {label}", round(global_diff - resumo_diff, 2)))
    ...
    def _to_excel_package(df_result, resumo_dict, text_priority_cols=None, totalizador_df=None, financial_kpis=None):
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            if totalizador_df is not None and not totalizador_df.empty:
                ...
                data_end = start_row + 1 + len(totalizador_df)
                for r in range(len(totalizador_df)):
                    for c, col in enumerate(totalizador_df.columns):
                        ...
                        if isinstance(val, (int, float, np.integer, np.floating)) and ("Diferença" in str(col) or nome_base_a in str(col) or nome_base_b in str(col)):
                            ws.write_number(data_start + r, c, float(round(val, 2)), fmt_num)
                        elif isinstance(val, (int, float, np.integer, np.floating)) and "Qtde" in str(col):
                            ws.write_number(data_start + r, c, float(val), fmt_value)
                        else:
                            ws.write(data_start + r, c, val, fmt_value)
                ws.autofilter(start_row + 1, 0, data_end, max(len(totalizador_df.columns) - 1, 0))
                subtotal_row = data_end + 1
                total_row = data_end + 2
                ws.write(subtotal_row, 0, "TOTAL FILTRADO", fmt_hdr)
                ...
                for c, col in enumerate(totalizador_df.columns[1:], start=1):
                    ...
                    last_row = data_end + 1
                    if "Diferença" in str(col) or nome_base_a in str(col) or nome_base_b in str(col):
                        ws.write_formula(subtotal_row, c, f"=SUBTOTAL(109,{col_letter}{first_row}:{col_letter}{last_row})", fmt_total)
                        ...
                        ws.write_formula(subtotal_row, c, f"=SUBTOTAL(109,{col_letter}{first_row}:{col_letter}{last_row})", fmt_kpi)
                        val = float(pd.to_numeric(totalizador_df[col], errors="coerce").fillna(0).sum())
    ...
    # Opções canônicas para agrupamento: campos-chave + associações confirmadas + campos adicionais das bases.
    ...
    for spec in association_specs:
        ...
        if src:
            _add_totalizador_option(_canon_totalizer_label(src, None), (nome_base_a, src))
        if map_col and dst:
            _add_totalizador_option(_canon_totalizer_label(dst, None), (nome_base_a, map_col))
            _add_totalizador_option(_canon_totalizer_label(dst, None), (nome_base_b, dst))

    for c in work_a.columns:
        _add_totalizador_option(_canon_totalizer_label(c, None), (nome_base_a, c))
    for c in work_b.columns:
        _add_totalizador_option(_canon_totalizer_label(c, None), (nome_base_b, c))

    group_options = list(totalizador_map.keys())
    # Evita estado inconsistente entre execuções/reloads.
    if "v14_group_choices" in st.session_state:
        st.session_state["v14_group_choices"] = [str(x) for x in st.session_state["v14_group_choices"] if x in group_options]
    ...
    for _, cp in selected_compares.iterrows():
        ...
        compare_value_map[lbl] = {
            "campo_a": cp["CAMPO_BASE_A"],
            "campo_b": cp["CAMPO_BASE_B"],
        }
        if lbl not in compare_value_options:
            compare_value_options.append(lbl)

    if "v14_value_choices" in st.session_state:
        st.session_state["v14_value_choices"] = [str(x) for x in st.session_state["v14_value_choices"] if x in compare_value_options]

    st.markdown("#### Como deseja montar o resumo executivo?")
    gx, vx = st.columns(2)
    with gx:
        if not st.session_state.get("v14_group_choices"):
            st.session_state["v14_group_choices"] = default_group_choices[:]
        group_choices = st.multiselect(
            "Agrupar resumo por",
            group_options,
            default=st.session_state.get("v14_group_choices", default_group_choices),
            help="Esses campos definem a chave de agrupamento do resumo executivo. Os campos usados para identificar o mesmo registro já vêm pré-selecionados.",
            key="v14_group_choices",
        )
    with vx:
        if not st.session_state.get("v14_value_choices"):
            st.session_state["v14_value_choices"] = compare_value_options[:]
        value_choices = st.multiselect(
            "O que deseja totalizar/confrontar",
            compare_value_options,
            default=st.session_state.get("v14_value_choices", compare_value_options),
            help="Esses são os campos de valor que serão somados e confrontados dentro do agrupamento escolhido.",
            key="v14_value_choices",
        )
    if not group_choices:
        group_choices = default_group_choices[:]
    ...
    with st.spinner("Processando análise..."):
        ...
        final_cols = list(dict.fromkeys([c for c in final_cols if isinstance(c, str) and c in df_result.columns]))
        df_result = df_result.loc[:, final_cols].copy()
        ...
        if gerar_resumo_exec:
            group_specs = []
            for choice in group_choices:
                tuples_choice = totalizador_map.get(choice, [])
                campo_a = next((campo for lado, campo in tuples_choice if lado == nome_base_a), None)
                campo_b = next((campo for lado, campo in tuples_choice if lado == nome_base_b), None)
                result_col = f"TOTALIZADOR__{choice}" if f"TOTALIZADOR__{choice}" in df_result.columns else None
                group_specs.append({"label": choice, "campo_a": campo_a, "campo_b": campo_b, "result_col": result_col})
            totalizador_df, financial_kpis = _build_totalizador(
                df_result,
                compare_meta,
                totalizador_result_cols,
                raw_base_a=base_a,
                raw_base_b=base_b,
                selected_compares_df=selected_compares,
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V15_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
