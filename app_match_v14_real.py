...
def render_cruzamento_inteligente_v2():
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols, raw_base_a=None, raw_base_b=None, selected_compares_df=None):
        if df_result is None or df_result.empty:
            return pd.DataFrame(), []
        work = df_result.copy()
        group_cols = [c for c in totalizador_cols if isinstance(c, str) and c in work.columns]
        if not group_cols:
            group_cols = [c for c in work.columns if isinstance(c, str) and c.startswith("TOTALIZADOR__")][:3]
        if not group_cols:
            return pd.DataFrame(), []

        selected_meta = [m for m in compare_meta if m.get("selected", True)] or list(compare_meta)

        # KPI financeiro deve fechar sempre com as bases inteiras, independentemente do agrupamento.
        financial_kpis = []
        compares_lookup = {}
        if selected_compares_df is not None and not selected_compares_df.empty:
        ...
        for meta in selected_meta:
            ...
            if cp is not None and raw_base_a is not None and raw_base_b is not None:
                va_all = _round_money_series(raw_base_a.get(cp["CAMPO_BASE_A"], pd.Series(dtype=object))).sum()
                vb_all = _round_money_series(raw_base_b.get(cp["CAMPO_BASE_B"], pd.Series(dtype=object))).sum()
            else:
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V14_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
