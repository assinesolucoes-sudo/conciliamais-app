...
st.set_page_config(page_title="Concilia Mais - Match Inteligente V26", layout="wide")
...
def _build_resumo_metricas(resumo_global: pd.DataFrame, detalhe: pd.DataFrame, ponte: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], List[str]]:
    ...
    int_inds = ["Campos confrontados", "Itens em divergência", f"Qtd. só na {base1}", f"Qtd. só na {base2}", "Qtd. em duplicidade"]
...
def _export_excel(resumo_global: pd.DataFrame, resumo_exec: pd.DataFrame, detalhe: pd.DataFrame, ponte: pd.DataFrame, regras_df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(bio, engine="xlsxwriter") as writer:
        ...
        # Regrava a coluna de valor dos indicadores com formatação mista (quantidade x monetário)
...
def main():
    _init_state()
    st.title("Concilia Mais - Match Inteligente V26")
