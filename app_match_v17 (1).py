...
def render_cruzamento_inteligente_v2():
    ...
    def _build_totalizador(df_result, compare_meta, totalizador_cols):
        ...
        def _num(sr):
            return pd.to_numeric(sr, errors="coerce").fillna(0).round(2)

        financial_kpis = []
        for meta in selected_meta:
            label = meta.get("label") or _label_exec(meta.get("diff_col"))
            va_all = _num(work.get(meta.get("val_a_col"))).sum()
            vb_all = _num(work.get(meta.get("val_b_col"))).sum()
        ...
        for keys, grp in grouped:
            ...
            row[f"Qtde {nome_base_a}"] = int((grp["CENARIO_MATCH"].astype(str) != f"Não encontrado na {nome_base_a}").sum()) if "CENARIO_MATCH" in grp.columns else int(len(grp))
            row[f"Qtde {nome_base_b}"] = int((grp["CENARIO_MATCH"].astype(str) != f"Não encontrado na {nome_base_b}").sum()) if "CENARIO_MATCH" in grp.columns else int(len(grp))
            ...
            row[f"Qtde só {nome_base_a}"] = int(grp["CENARIO_MATCH"].astype(str).eq(f"Não encontrado na {nome_base_b}").sum()) if "CENARIO_MATCH" in grp.columns else 0
            row[f"Qtde só {nome_base_b}"] = int(grp["CENARIO_MATCH"].astype(str).eq(f"Não encontrado na {nome_base_a}").sum()) if "CENARIO_MATCH" in grp.columns else 0
            ...
            motivos = []
            if row["Qtde Divergências"]:
                motivos.append("Diferença de valor")
            if row.get(f"Qtde só {nome_base_a}",0):
                motivos.append(f"Só {nome_base_a}")
            if row.get(f"Qtde só {nome_base_b}",0):
                motivos.append(f"Só {nome_base_b}")
            if row["Qtde Duplicidades"]:
                motivos.append("Duplicidade")
            row["Motivo"] = " | ".join(motivos) if motivos else "Sem divergência"
            for meta in selected_meta:
                label = meta.get("label") or _label_exec(meta.get("diff_col"))
                va = _num(grp.get(meta.get("val_a_col"))).sum()
                vb = _num(grp.get(meta.get("val_b_col"))).sum()
        ...
        support_cols = [c for c in ["Qtde Divergências", f"Qtde só {nome_base_a}", f"Qtde só {nome_base_b}", "Qtde Duplicidades"] if c in agg.columns]
        if diff_cols or support_cols:
            mask_keep = agg[diff_cols + support_cols].apply(pd.to_numeric, errors="coerce").fillna(0).abs().sum(axis=1) > 0
            if mask_keep.any():
                agg = agg[mask_keep].copy()
        if not agg.empty:
            preferred = group_labels + [c for c in agg.columns if c not in group_labels]
            agg = agg[preferred].sort_values(group_labels).reset_index(drop=True)
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            if totalizador_df is not None and not totalizador_df.empty:
                ...
                for c, col in enumerate(totalizador_df.columns):
                    ...
                    ws.set_column(c, c, width)
                qty_cols = [c for c in totalizador_df.columns if str(c).startswith("Qtde")]
                money_cols = [
                    c for c in totalizador_df.columns
                    if c not in qty_cols and (
                        str(c).startswith("Diferença ") or
                        str(c).endswith(f" {nome_base_a}") or
                        str(c).endswith(f" {nome_base_b}")
                    )
                ]
                for r in range(len(totalizador_df)):
                    for c, col in enumerate(totalizador_df.columns):
                        val = totalizador_df.iloc[r, c]
                        if col in money_cols and pd.notna(pd.to_numeric(pd.Series([val]), errors="coerce").iloc[0]):
                            ws.write_number(start_row + 2 + r, c, float(round(pd.to_numeric(val, errors="coerce"), 2)), fmt_num)
                        elif col in qty_cols and pd.notna(pd.to_numeric(pd.Series([val]), errors="coerce").iloc[0]):
                            ws.write_number(start_row + 2 + r, c, float(pd.to_numeric(val, errors="coerce")), fmt_value)
                        else:
                ...
                for c, col in enumerate(totalizador_df.columns[1:], start=1):
                    if col in money_cols:
                        ...
                        ws.write_blank(total_row, c, None, fmt_value)
    ...
    # Agrupadores: somente campos-chave do match e campos de associação confirmados.
    ...
    for spec in association_specs:
        ...
        if src and dst:
            lbl = f"[MAP] {_canon_totalizer_label(src, dst)}"
            if map_col:
                _add_totalizador_option(lbl, (nome_base_a, map_col))
            else:
                _add_totalizador_option(lbl, (nome_base_a, src))
    ...
    for spec in association_specs:
        ...
        if src and dst:
            ...
            if lbl in group_options and lbl not in default_group_choices:
                default_group_choices.append(lbl)
    if not default_group_choices:
        default_group_choices = group_options[:]

    compare_value_options = []
    compare_value_map = {}
    for _, cp in selected_compares.iterrows():
        lbl = str(cp["CAMPO_BASE_A"]).strip() or _canon_totalizer_label(cp["CAMPO_BASE_A"], cp["CAMPO_BASE_B"])
    ...
    # saneia restos de sessao de versoes anteriores / selecoes invalidas
    existing_group_state = st.session_state.get("v17_group_choices", default_group_choices[:])
    if not isinstance(existing_group_state, list):
        existing_group_state = default_group_choices[:]
    existing_group_state = [x for x in existing_group_state if x in group_options]
    if not existing_group_state:
        existing_group_state = default_group_choices[:]

    existing_value_state = st.session_state.get("v17_value_choices", compare_value_options[:])
    if not isinstance(existing_value_state, list):
        existing_value_state = compare_value_options[:]
    existing_value_state = [x for x in existing_value_state if x in compare_value_options]
    if not existing_value_state:
        existing_value_state = compare_value_options[:]
    ...
    with gx:
        group_choices = st.multiselect(
            "Agrupar resumo por",
            group_options,
            default=existing_group_state,
            help="Use aqui os campos-chave do match e, se desejar, campos de associação para explicar onde está a diferença.",
            key="v17_group_choices",
        )
    with vx:
        value_choices = st.multiselect(
            "O que deseja totalizar/confrontar",
            compare_value_options,
            default=existing_value_state,
            help="Esses são apenas os campos de valor marcados para confronto.",
            key="v17_value_choices",
        )
    ...
    with st.spinner("Processando análise..."):
        ...
        for choice in group_choices:
            ...
            # Mantém uma única coluna canônica para agrupar, sempre preenchendo por coalescência entre as bases.
            candidate_cols = []
            for lado, campo in source_tuples:
                ...
                candidate_cols.append(_force_text_series(col).str.strip())
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name=f"Match_Inteligente_V17_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
